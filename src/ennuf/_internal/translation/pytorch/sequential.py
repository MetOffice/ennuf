...
def from_sequential(
    ...
    for i, layer in enumerate(original_layers):
        ...
        _process_layer(ennuf_model, layer, layer_name, processed_layers, i)
...
def _process_layer(ennuf_model, layer, layer_name, processed_layers, i):
    if processed_layers == []:
        input_name = "input"
    else:
    ...
    if isinstance(layer, nn.Linear):
        ...
        if pool_size is None:
            raise NotImplementedError(
                f"Unsupported pool size, expected 1d pool size shape but got {layer.kernel_size}"
            )
        type_of_pooling = "MAX" if isinstance(layer, nn.MaxPool1d) else "AVG"
        padding = (
            layer.padding
            if isinstance(layer.padding, int)
            else layer.padding[0] if len(layer.padding) == 1 else None
        )
        if padding is None:
            raise NotImplementedError(f"Unsupported padding, expected 1d padding shape but got {layer.padding}")
        stride = (
            layer.stride if isinstance(layer.stride, int) else layer.stride[0] if len(layer.stride) == 1 else None
        )
        if stride is None:
            raise NotImplementedError(f"Unsupported stride, expected 1d padding stride but got {layer.stride}")
        ...
        for j, sublayer in enumerate(layer.children()):
            _process_layer(ennuf_model, sublayer, str(sublayer).split("(")[0] + str(i) + str(j), processed_layers, i)
    else:

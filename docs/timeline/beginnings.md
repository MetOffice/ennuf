...
ENNUF was born in early 2023 out of the need for a temporary solution to deal with the
fact that we currently 
have no good way of running machine learning models (which are typically designed and
trained in python using libraries such as TensorFlow or PyTorch) in the UM 
(which is of course written in Fortran). Calling python code from the UM has been
tried and massively slows it down.
...
ENNUF is an apt name - it may be an incredibly janky solution to this complex problem,
perhaps wonderfully so, but it's *good ennuf* to use for our purposes until such a
time as a better solution exists.